...
from sirius.blueprints.api.remote_service.tula.passive.hospitalization.xform import \
    HospitalizationTulaXForm
...
@module.route('/api/integration/<int:api_version>/card/<' + parent_id_name + '>/measures/hospitalization',
              methods=['POST'])
@module.route('/api/integration/<int:api_version>/card/<' + parent_id_name + '>/measures/hospitalization/<' + main_id_name + '>',
              methods=['PUT', 'DELETE'])
@remote_api_method(hook=hook)
def api_hospitalization_change(api_version, **kwargs):
    main_id = kwargs.get(main_id_name)
    parent_id = kwargs.get(parent_id_name)
    stream_id = kwargs.get('stream_id')
    data = None
    delete = request.method == 'DELETE'
    xform = HospitalizationTulaXForm(api_version, stream_id)
    if not delete:
