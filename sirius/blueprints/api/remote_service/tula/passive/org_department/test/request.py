...
def create_org_department(testapp, session, data):
    url = u'/tula/api/integration/0/organization_department/'
    # result = make_test_api_request(testapp, 'post', url, session, data)
    result = make_api_request('post', url, session, data)
    return result


def edit_org_department(testapp, session, main_id, data):
    url = u'/tula/api/integration/0/organization_department/%s' % main_id
    # result = make_test_api_request(testapp, 'put', url, session, data)
    result = make_api_request('put', url, session, data)
    return result


def delete_org_department(testapp, session, main_id):
    url = u'/tula/api/integration/0/organization_department/%s' % main_id
