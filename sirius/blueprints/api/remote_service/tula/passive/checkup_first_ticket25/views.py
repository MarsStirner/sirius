...
@module.route('/api/integration/<int:api_version>/card/<' + parent_id_name + '>/checkup/obs/first/<' + main_id_name + '>/ticket25',
              methods=['PUT'])
...
def api_checkup_first_ticket25_change(api_version, **kwargs):
    ...
    xform = CheckupFirstTicket25TulaXForm(api_version, stream_id)
    if not delete:
