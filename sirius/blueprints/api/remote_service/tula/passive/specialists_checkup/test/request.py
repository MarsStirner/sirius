...
def create_sp_checkup(testapp, session, parent_id, data):
    url = u'/tula/api/integration/0/card/%s/measures/specialists_checkup' % parent_id
    # result = make_test_api_request(testapp, 'post', url, session, data)
    result = make_api_request('post', url, session, data)
    return result


def edit_sp_checkup(testapp, session, parent_id, main_id, data):
    url = u'/tula/api/integration/0/card/%s/measures/specialists_checkup/%s' % (parent_id, main_id)
    # result = make_test_api_request(testapp, 'put', url, session, data)
    result = make_api_request('put', url, session, data)
    return result


def delete_sp_checkup(testapp, session, parent_id, main_id):
    url = u'/tula/api/integration/0/card/%s/measures/specialists_checkup/%s' % (parent_id, main_id)
